...
struct MeshData {
  ...
  MeshData(const std::string &name, const pcl::PolygonMesh &mesh,
           const Eigen::Vector3d &centroid)
      : name(name), mesh(mesh), centroid(centroid){};
};

typedef std::vector<MeshData, Eigen::aligned_allocator<MeshData>>
    MeshDataVector;
...
int main(int argc, char **argv) {
  ...
  const std::string outputPath =
      (argc == 4) ? std::string(argv[3]) : sourcePath;
  ...
  for (const auto &sourceMeshData : sourceDataVec) {
    ...
    const auto targetMeshDataIt = std::min_element(
        targetDataVec.begin(), targetDataVec.end(),
    ...
    const std::string heatmapFilename =
        outputPath +
        std::string(sourceMeshData.name.begin(),
                    sourceMeshData.name.end() - 4) +
        "_heatmap.ply";
    ...
    meshDifference.saveHeatmapMesh(heatmapFilename, minDistance, maxDistance,
                                   colormap);
  }
