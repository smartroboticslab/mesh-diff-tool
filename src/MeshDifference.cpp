...
MeshDifference::sampleSurfaceMesh(const pcl::PolygonMesh &mesh,
                                  const double samplingDensity) {
  pcl::PointCloud<pcl::PointXYZ>::Ptr sampledPCPtr(
      new pcl::PointCloud<pcl::PointXYZ>());

  // Convert the Input mesh to VTK.
  vtkSmartPointer<vtkPolyData> polygonDataPtr =
...
double MeshDifference::computeTotalArea(
    const vtkSmartPointer<vtkPolyData> polygonDataPtr) {
  double result = 0;
  if (polygonDataPtr->NeedToBuildCells())
    polygonDataPtr->BuildCells();
...
void MeshDifference::sampleTriangleSurface(
...
void MeshDifference::saveHeatmapMesh(const std::string &filename,
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr heatmapPCPtr(
  ...
  for (const auto &heatmapEntry : meshDifference_) {
    ...
    const double scaledDistance = std::max(
        std::min((distance - minDistance) / (maxDistance - minDistance), 1.0),
        0.0);
